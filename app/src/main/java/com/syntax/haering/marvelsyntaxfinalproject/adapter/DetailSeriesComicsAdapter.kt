...
import com.syntax.haering.marvelsyntaxfinalproject.ui.DetailSerieFragmentDirections

class DetailSeriesComicsAdapter: RecyclerView.Adapter<DetailSeriesComicsAdapter.ItemViewHolder>() {
    ...
    inner class ItemViewHolder(val binding: ItemDetailCardBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(comics: com.syntax.haering.marvelsyntaxfinalproject.data.importComicData.Result){
            binding.detailCardCv.setOnClickListener {
                Navigation.findNavController(itemView).navigate(DetailSerieFragmentDirections.actionDetailSerieFragmentToDetailComicFragment(comics.id))
            }
